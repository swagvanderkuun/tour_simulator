...
class TNOTeamOptimizer:
    ...
    def _optimize_rider_selection(self, rider_data: pd.DataFrame, 
        ...
        # Constraint 3: Maximum 4 riders per team
        team_counts = {}
        for _, row in rider_data.iterrows():
            rider_name = row['rider_name']
            team = row['team']
...
def main():
    ...
    print("=== TNO-Ergame Team Optimizer ===")
    ...
    print(f"Analyzed {len(rider_data)} riders")
    print(f"Top 5 expected performers:")
    for i, (_, row) in enumerate(rider_data.head().iterrows(), 1):
        print(f"{i}. {row['rider_name']}: {row['expected_points']:.1f} points")
    
    # Optimize team
    print("\nOptimizing team selection and order...")
    ...
    print(f"Alternative teams:")
    for i, alt in enumerate(alternatives, 1):
        print(f"Alt {i}: {alt.expected_points:.1f} points, Cost: {alt.total_cost:.2f}")
